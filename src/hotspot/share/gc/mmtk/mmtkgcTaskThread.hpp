...
private:
  ...
  static mmtkGCTaskThread* create(mmtkGCTaskManager* manager,
                              uint           which,
                              uint           processor_id) {
    return new mmtkGCTaskThread(manager, which, processor_id);
  }
 public:

  static void destroy(mmtkGCTaskThread* manager) {
    if (manager != NULL) {
      delete manager;
    }
  }
...
protected:
  ...
  mmtkGCTaskThread(mmtkGCTaskManager* manager, uint which, uint processor_id);
  // Destructor: virtual destructor because of virtual methods.
  virtual ~mmtkGCTaskThread();
  // Accessors.
  mmtkGCTaskManager* manager() const {
    return _manager;
  }
  uint which() const {
    return id();
...
class mmtkGCTaskTimeStamp : public CHeapObj<mtGC>
{
 private:
  jlong  _entry_time;
  jlong  _exit_time;
