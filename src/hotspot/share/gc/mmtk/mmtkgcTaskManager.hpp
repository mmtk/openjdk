...
#ifndef SHARE_VM_GC_PARALLEL_MMTKGCTASKMANAGER_HPP
#define SHARE_VM_GC_PARALLEL_MMTKGCTASKMANAGER_HPP

#include "runtime/mutex.hpp"
#include "utilities/growableArray.hpp"

//
// The mmtkGCTaskManager is a queue of GCTasks, and accessors
// to allow the queue to be accessed from many threads.
//

// Forward declarations of types defined in this file.
class mmtkGCTask;
class mmtkGCTaskQueue;
class mmtkSynchronizedGCTaskQueue;
class mmtkGCTaskManager;
// Some useful subclasses of mmtkGCTask.  You can also make up your own.
class mmtkNoopGCTask;
class mmtkWaitForBarrierGCTask;
class mmtkIdleGCTask;
// A free list of Monitor*'s.
class mmtkMonitorSupply;

// Forward declarations of classes referenced in this file via pointer.
class mmtkGCTaskThread;
class Mutex;
class Monitor;
class ThreadClosure;

// The abstract base mmtkGCTask.
class mmtkGCTask : public ResourceObj {
public:
...
private:
  ...
  mmtkGCTask*          _newer;              // Tasks are on doubly-linked ...
  mmtkGCTask*          _older;              // ... lists.
...
public:
  ...
  virtual void do_it(mmtkGCTaskManager* manager, uint which) = 0;
  // Accessors
  ...
  mmtkGCTask* newer() const {
    return _newer;
  }
  void set_newer(mmtkGCTask* n) {
    _newer = n;
  }
  mmtkGCTask* older() const {
    return _older;
  }
  void set_older(mmtkGCTask* p) {
    _older = p;
  }
  // Predicates.
...
protected:
  ...
  //     An ordinary mmtkGCTask.
  mmtkGCTask();
  //     A mmtkGCTask of a particular kind, usually barrier or noop.
  mmtkGCTask(Kind::kind kind);
  mmtkGCTask(Kind::kind kind, uint gc_id);
...
// See also: mmtkSynchronizedGCTaskQueue.
class mmtkGCTaskQueue : public ResourceObj {
private:
  // Instance state.
  mmtkGCTask*    _insert_end;               // Tasks are enqueued at this end.
  mmtkGCTask*    _remove_end;               // Tasks are dequeued from this end.
  uint       _length;                   // The current length of the queue.
  const bool _is_c_heap_obj;            // Is this a CHeapObj?
public:
  // Factory create and destroy methods.
  //     Create as ResourceObj.
  static mmtkGCTaskQueue* create();
  //     Create as CHeapObj.
  static mmtkGCTaskQueue* create_on_c_heap();
  //     Destroyer.
  static void destroy(mmtkGCTaskQueue* that);
  // Accessors.
  ...
  // Methods.
  //     Enqueue one task.
  void enqueue(mmtkGCTask* task);
  //     Enqueue a list of tasks.  Empties the argument list.
  void enqueue(mmtkGCTaskQueue* list);
  //     Dequeue one task.
  mmtkGCTask* dequeue();
  //     Dequeue one task, preferring one with affinity.
  mmtkGCTask* dequeue(uint affinity);
protected:
  // Constructor. Clients use factory, but there might be subclasses.
  mmtkGCTaskQueue(bool on_c_heap);
  // Destructor-like method.
  // Because ResourceMark doesn't call destructors.
  // This method cleans up like one.
  virtual void destruct();
  // Accessors.
  mmtkGCTask* insert_end() const {
    return _insert_end;
  }
  void set_insert_end(mmtkGCTask* value) {
    _insert_end = value;
  }
  mmtkGCTask* remove_end() const {
    return _remove_end;
  }
  void set_remove_end(mmtkGCTask* value) {
    _remove_end = value;
  }
  ...
  // Methods.
  void initialize();
  mmtkGCTask* remove();                     // Remove from remove end.
  mmtkGCTask* remove(mmtkGCTask* task);         // Remove from the middle.
  void print(const char* message) const PRODUCT_RETURN;
  // Debug support
  void verify_length() const PRODUCT_RETURN;
};

// A mmtkGCTaskQueue that can be synchronized.
// This "has-a" mmtkGCTaskQueue and a mutex to do the exclusion.
class mmtkSynchronizedGCTaskQueue : public CHeapObj<mtGC> {
private:
  // Instance state.
  mmtkGCTaskQueue* _unsynchronized_queue;   // Has-a unsynchronized queue.
  Monitor *    _lock;                   // Lock to control access.
public:
  // Factory create and destroy methods.
  static mmtkSynchronizedGCTaskQueue* create(mmtkGCTaskQueue* queue, Monitor * lock) {
    return new mmtkSynchronizedGCTaskQueue(queue, lock);
  }
  static void destroy(mmtkSynchronizedGCTaskQueue* that) {
    if (that != NULL) {
      delete that;
    }
  }
  // Accessors
  mmtkGCTaskQueue* unsynchronized_queue() const {
    return _unsynchronized_queue;
  }
  Monitor * lock() const {
    return _lock;
  }
  // mmtkGCTaskQueue wrapper methods.
...
protected:
  // Constructor.  Clients use factory, but there might be subclasses.
  mmtkSynchronizedGCTaskQueue(mmtkGCTaskQueue* queue, Monitor * lock);
...
// mmtkGCTaskManager.  The GC threads that have been "idled" in a mmtkIdleGCTask
// are released once all the active GC threads have finished their work
// stealing tasks.  The mmtkGCTaskManager does not wait for all the "idled"
...
// held in the mmtkGCTaskThread** _thread array in mmtkGCTaskManager.


class mmtkGCTaskManager : public CHeapObj<mtGC> {
 friend class ParCompactionManager;
 friend class PSParallelCompact;
 friend class PSScavenge;
 friend class PSRefProcTaskExecutor;
 friend class RefProcTaskExecutor;
 friend class mmtkGCTaskThread;
 friend class mmtkIdleGCTask;
private:
  // Instance state.
  const uint                _workers;           // Number of workers.
  Monitor*                  _monitor;           // Notification of changes.
  mmtkSynchronizedGCTaskQueue*  _queue;             // Queue of tasks.
  mmtkGCTaskThread**            _thread;            // Array of worker threads.
  ...
  mmtkNoopGCTask*               _noop_task;         // The mmtkNoopGCTask instance.
...
public:
  // Factory create and destroy methods.
  static mmtkGCTaskManager* create(uint workers) {
    return new mmtkGCTaskManager(workers);
  }
  static void destroy(mmtkGCTaskManager* that) {
    if (that != NULL) {
      delete that;
    }
  }
  ...
  void add_task(mmtkGCTask* task);
  //     Add a list of tasks.  Removes task from the argument list.
  void add_list(mmtkGCTaskQueue* list);
  //     Claim a task for argument worker.
  mmtkGCTask* get_task(uint which);
  ...
  void execute_and_wait(mmtkGCTaskQueue* list);
...
protected:
  ...
  //     Create a mmtkGCTaskManager with the appropriate number of workers.
  mmtkGCTaskManager(uint workers);
  ...
  mmtkSynchronizedGCTaskQueue* queue() const {
    return _queue;
  }
  mmtkNoopGCTask* noop_task() const {
    return _noop_task;
  }
  //     Bounds-checking per-thread data accessors.
  mmtkGCTaskThread* thread(uint which);
  void set_thread(uint which, mmtkGCTaskThread* value);
 ...
 public:
  ...
  // Create a GC worker and install into mmtkGCTaskManager
  mmtkGCTaskThread* install_worker(uint worker_id);
...
// except take us around the mmtkGCTaskThread loop.
class mmtkNoopGCTask : public mmtkGCTask {
public:
  // Factory create and destroy methods.
  static mmtkNoopGCTask* create_on_c_heap();
  static void destroy(mmtkNoopGCTask* that);

  virtual char* name() { return (char *)"noop task"; }
  // Methods from mmtkGCTask.
  void do_it(mmtkGCTaskManager* manager, uint which) {
    // Nothing to do.
  }
protected:
  // Constructor.
...
// A mmtkWaitForBarrierGCTask is a mmtkGCTask
// with a method you can call to wait until
// the BarrierGCTask is done.
class mmtkWaitForBarrierGCTask : public mmtkGCTask {
  friend class mmtkGCTaskManager;
  friend class mmtkIdleGCTask;
private:
  // Instance state.
...
public:
  ...
  // Factory create and destroy methods.
  static mmtkWaitForBarrierGCTask* create();
  static void destroy(mmtkWaitForBarrierGCTask* that);
  // Methods.
  void     do_it(mmtkGCTaskManager* manager, uint which);
protected:
  // Destructor-like method.
  ...
  void do_it_internal(mmtkGCTaskManager* manager, uint which);
...
class mmtkIdleGCTask : public mmtkGCTask {
 ...
 public:
  ...
  // Factory create and destroy methods.
  static mmtkIdleGCTask* create();
  static mmtkIdleGCTask* create_on_c_heap();
  static void destroy(mmtkIdleGCTask* that);

  virtual char* name() { return (char *)"idle task"; }
  // Methods from mmtkGCTask.
  virtual void do_it(mmtkGCTaskManager* manager, uint which);
protected:
  // Constructor.
  mmtkIdleGCTask(bool on_c_heap) :
    mmtkGCTask(mmtkGCTask::Kind::idle_task),
...
class mmtkMonitorSupply : public AllStatic {
private:
  // State.
