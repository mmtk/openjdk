...
mmtkGCTaskThread::mmtkGCTaskThread(mmtkGCTaskManager* manager,
                           uint           which,
                           uint           processor_id) :
  _manager(manager),
...
void mmtkGCTaskThread::print_task_time_stamps() {
...
// GC workers get tasks from the mmtkGCTaskManager and execute
// them in this method.  If there are no tasks to execute,
// the GC workers wait in the mmtkGCTaskManager's get_task()
// for tasks to be enqueued for execution.

void mmtkGCTaskThread::run() {
  // Set up the thread for stack overflow support
  this->record_stack_base_and_size();
  this->initialize_named_thread();
  // Bind yourself to your processor.
  if (processor_id() != mmtkGCTaskManager::sentinel_worker()) {
    log_trace(gc, task, thread)("mmtkGCTaskThread::run: binding to processor %u", processor_id());
    if (!os::bind_to_processor(processor_id())) {
      DEBUG_ONLY(
        log_warning(gc)("Couldn't bind mmtkGCTaskThread %u to processor %u",
                        which(), processor_id());
      )
    }
  }
  ...
  for (;/* ever */;) {
    ...
    for (; /* break */; ) {
      ...
      if (!is_idle_task) {
        ...
        mmtkIdleGCTask::destroy((mmtkIdleGCTask*)task);
        set_is_working(true);
      }
